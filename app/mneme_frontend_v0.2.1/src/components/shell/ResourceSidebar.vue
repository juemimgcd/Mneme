...
const overlayQuery = window.matchMedia("(max-width: 1024px)");
...
<template>
  <Transition name="resource-scrim">
    <button v-if="open" class="resource-sidebar__scrim" type="button" :aria-label="t('shell.closeResources')" @click="emit('close')" />
  </Transition>
  <aside
