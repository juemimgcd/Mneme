...
defineProps<{ items: NavigationItem[]; activeId: string; moreActive: boolean }>();
...
<template>
  <nav data-testid="mobile-navigation" class="mobile-navigation" aria-label="Mobile workspace">
    ...
    <button
      type="button"
      :class="{ 'mobile-navigation__active': moreActive }"
      aria-label="Open more navigation"
      :aria-expanded="moreActive"
      aria-controls="more-navigation-sheet"
      @click="emit('openMore')"
    ><Ellipsis class="size-[18px]" /><span>More</span></button>
  </nav>
</template>

<style scoped>
.mobile-navigation { position: fixed; inset: auto 0 0; z-index: 35; display: none; min-height: 3.7rem; grid-template-columns: repeat(5, minmax(0, 1fr)); padding: 0.3rem max(0.35rem, env(safe-area-inset-right)) max(0.35rem, env(safe-area-inset-bottom)) max(0.35rem, env(safe-area-inset-left)); background: color-mix(in srgb, var(--bg-sidebar) 96%, transparent); border-top: 1px solid var(--border-muted); }
