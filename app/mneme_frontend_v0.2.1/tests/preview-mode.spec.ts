...
import { getDocumentAction, openMoreAction, openView } from './helpers/navigation';
...
test('preview mode opens the populated workbench without a backend login', async ({ page }) => {
  ...
  const graphRail = page.getByTestId('graph-file-rail');
  const viewport = page.viewportSize();

  if (viewport && viewport.width < 1024) {
    await expect(graphRail).toBeHidden();
  } else {
    await expect(graphRail).toBeVisible();
    await expect(graphRail).toContainText('memory-graph-design.pdf');
  }
...
test('research vault upload and document actions are wired', async ({ page }) => {
  ...
  await (await getDocumentAction(page, 'Delete')).click();
  await page.getByRole('dialog', { name: 'Delete this document?' }).getByRole('button', { name: 'Delete', exact: true }).click();
...
test('knowledge graph document panel opens on click and canvas clears selection', async ({ page }) => {
  ...
  if (viewport && viewport.width <= 820) {
    await panel.getByRole('button', { name: 'Close graph preview' }).click();
  } else {
...
test('ai chat history rail can collapse and expand from the chat handle', async ({ page }) => {
  ...
  if (viewport && viewport.width < 1024) {
    await rail.getByRole('button', { name: 'Close chat history' }).click();
  } else {
    await toggle.click();
  }
  await expect(rail).toBeHidden();
...
test('ai laboratory filters and deletes chat sessions', async ({ page }) => {
  ...
  if (viewport && viewport.width < 1024) {
    await page.getByTestId('ai-history-rail').getByRole('button', { name: 'Close chat history' }).click();
  }
